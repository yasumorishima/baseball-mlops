...
//
// 12 skill groups:
//   FG/Savant: stuff, command, contact_mgmt, arsenal, context, era_models, batted_ball_fg, role
//   BQ:       velo_bq, command_bq, contact_bq, fatigue_bq
//
// xFIP is LOWER = BETTER
...
data {
  ...
  int<lower=1> K_era_models;
  int<lower=1> K_batted_ball_fg;
  int<lower=1> K_role;
  int<lower=1> K_velo_bq;
  int<lower=1> K_command_bq;
  int<lower=1> K_contact_bq;
  int<lower=1> K_fatigue_bq;
  ...
  matrix[N, K_era_models] X_era_models;
  matrix[N, K_batted_ball_fg] X_batted_ball_fg;
  matrix[N, K_role] X_role;
  ...
  matrix[N_pred, K_era_models] X_era_models_pred;
  matrix[N_pred, K_batted_ball_fg] X_batted_ball_fg_pred;
  matrix[N_pred, K_role] X_role_pred;
...
parameters {
  ...
  real<lower=0> tau_era_models;
  vector[K_era_models] z_beta_era_models;
  real<lower=0> tau_batted_ball_fg;
  vector[K_batted_ball_fg] z_beta_batted_ball_fg;
  real<lower=0> tau_role;
  vector[K_role] z_beta_role;
...
transformed parameters {
  ...
  vector[K_era_models] beta_era_models = tau_era_models * z_beta_era_models;
  vector[K_batted_ball_fg] beta_batted_ball_fg = tau_batted_ball_fg * z_beta_batted_ball_fg;
  vector[K_role] beta_role = tau_role * z_beta_role;
...
model {
  ...
  // ERA models: SIERA/ERA-/FIP-/xFIP- are strong xFIP predictors
  tau_era_models ~ normal(0, 0.10);
  // FG batted ball: GB/FB/LD/HR/FB distribution
  tau_batted_ball_fg ~ normal(0, 0.06);
  // Role: starter vs reliever affects xFIP baseline
  tau_role ~ normal(0, 0.04);
  ...
  z_beta_era_models ~ std_normal();
  z_beta_batted_ball_fg ~ std_normal();
  z_beta_role ~ std_normal();
...
generated quantities {
  ...
  for (i in 1:N_pred) {
    real mu_i = marcel_pred[i]
                ...
                + X_era_models_pred[i] * beta_era_models
                + X_batted_ball_fg_pred[i] * beta_batted_ball_fg
                + X_role_pred[i] * beta_role
  ...
  for (n in 1:N) {
    real mu_n = marcel[n]
                ...
                + X_era_models[n] * beta_era_models
